...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  ...
  Args:
    ...
    initial_state_attention:
      ...
      through a linear layer with the previous step's context vector to get a modified version of the input.
  ...
  Returns:
    outputs: A list of the same length as decoder_inputs of 2D Tensors of HS: output = st라고 생각하면된다
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    # HS: attention size는 attention 을 구하기 위한 모델의 복잡도를 결정하기 위한 size라고 볼 수 있고
    # 임의로  W_h를 정방행렬로 만들어주기 위해 그냥 attn size를 hi의 사이즈로 잡은 것
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          if use_coverage: # first step of training
            coverage = tf.expand_dims(tf.expand_dims(attn_dist,2),2) # initialize coverage => HS: batch_size * att_length * 1 * 1이 됨
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      sq_encoder_states = tf.squeeze(encoder_states,axis=2)
      print("encoder_states.get_shape().as_list()",sq_encoder_states.get_shape().as_list())
      tp_sq_encoder_states = tf.transpose(sq_encoder_states,perm=[1,0,2])
      print("tp_sq_encoder_states", tp_sq_encoder_states.get_shape().as_list())
      encoder_state_list =  [x for x in tf.unstack(tp_sq_encoder_states, axis=0)]
      print("context_vecotr", context_vector.get_shape().as_list())

      enc_feature_list = []

      for j,enc_state in enumerate(encoder_state_list):
        print(j,"Im the body of enumer(encoder_state_list)")
        print("enc_state",enc_state.get_shape().as_list())
        with variable_scope.variable_scope("AttnOutputProjection_",reuse=tf.AUTO_REUSE):
          tmp = linear([cell_output] + [enc_state], cell.output_size, True) # 16 X 256
          enc_feature_list.append(tmp)
      outputs_.append(enc_feature_list) # 10 x 16 x 256
    ...
    # print("outputs",len(outputs_[0]))
    return outputs_, state, attn_dists, p_gens, coverage
